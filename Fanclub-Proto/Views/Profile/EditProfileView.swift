                ...
                HStack {
                    ...
                    Button {
                        ...
                        if vm.isLoading {
                            ...
                            Text("Save")
                                .foregroundStyle(vm.saveButtonDisabled() ? .gray : GlobalVars.YellowText)
                                .font(.custom("Nunito-Regular", size: 20))
                        }
                    }.disabled(vm.isLoading /*|| vm.usernameError*/ || vm.saveButtonDisabled())
                }.padding()
                ScrollView {
                    ...
                    Group {
                        ...
                        Group {
                            ...
                            Rectangle()
                                ...
                                .foregroundStyle(.white.opacity(0.65))
                                .padding(.bottom).padding(.horizontal)
//                            if vm.newUsername == "" && vm.emptyFieldError {
...
//                                VStack {
//                                    HStack {
//                                        TextField("1234567890", text: $vm.newPhone)
//                                            .foregroundStyle(.white)
//                                            .font(.custom("Montserrat-Regular", size: 16))
//                                            .textInputAutocapitalization(.never)
//                                            .autocorrectionDisabled()
...
//                                        .padding(.bottom).padding(.horizontal)
//                                }
//                                if (vm.newPhone != profileManager.profile?.mobile ?? "") && (vm.newPhone != "") && !(vm.verifiedNums.contains(vm.newPhone)) {
//                                    Button {
//                                        Task { try await vm.sendVerifyPhone() }
//                                    } label: {
//                                        Text("Send Code")
//                                    }.padding(.trailing)
//                                }
...
//                                    VStack {
//                                        HStack {
//                                            TextField("Enter the OTP code", text: $vm.otpCode)
//                                                .foregroundStyle(.white)
//                                                .font(.custom("Montserrat-Regular", size: 16))
//                                                .textInputAutocapitalization(.never)
//                                                .autocorrectionDisabled()
...
//                        }.disabled(vm.isLoading)
        ...
        .onChange(of: vm.newUsername) {
            vm.newUsername = vm.newUsername.replacingOccurrences(of: " ", with: "").lowercased()
            if vm.newUsername.count > 32 {
                vm.newUsername.removeLast()
            }
            
//            if vm.newUsername.isEmpty {
...
//            }
        ...
        .alert("Update profile failed", isPresented: $vm.updateError) {
            ...
            //TODO: -> if error returned from server set message here to message in error?
            Text("Please try again.")
        }
        .onChange(of: vm.newPhone) {
            if vm.verifiedNums.contains(vm.newPhone) || vm.newPhone == "" {
                vm.sentCode = false
                vm.otpCode = ""
            }
        }
